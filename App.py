...
from pdfminer3.layout import LTTextBox
...
def get_table_download_link(df,filename, text):
    """Generate a link allowing the data in a given panda dataframe to be download
    in : dataframe
    out: href string """
...
def course_recommender(course_list):
    st.subheader('**Courses & Certificates🎓Recommendation**')
    c = 0
    rec_courses = []  # Correct variable name
    no_of_reco = st.slider('Choose Number of Course Recommendation:', 1, 10, 5)
    random.shuffle(course_list)
    for c_name, c_link in course_list:
        c += 1
        st.markdown(f"({c}) [{c_name}]({c_link})")
        rec_courses.append(c_name)  # Corrected variable name
        if c == no_of_reco:
            break
    return rec_courses
...
st.set_page_config(
    ...
    page_icon="C:/NLP(PROJECT)_RESUME_ANALYSER/Logo.png",
)
...
def run():
    ...
    choice = st.sidebar.selectbox("Choose among the given opition", activities)
    img = Image.open("C:/GSFCU CSE/NLP(PROJECT)_RESUME_ANALYSER/NLP(PROJECT)_RESUME_ANALYSER/Logo.png")
...
choice = st.selectbox("Select User Type", ["Normal User", "Other User"])
if choice == 'Normal User':
    st.markdown('''<h4 styles= 'text-align: left; color: #d73b5c;>* Upload your Resume, and get smart recommendation based on it.</h4> ''',unsafe_allow_html=True)
    pdf_file = st.file_uploader("Choose your Resume", type=["pdf"])
    if pdf_file is not None:
        #with st.spinner('Uploading your Resume...'):
        #       time.sleep(4)
        save_image_path = "C:/GSFCU CSE/NLP(PROJECT)_RESUME_ANALYSER/NLP(PROJECT)_RESUME_ANALYSER/Upload_Resume" + pdf_file.name
        with open(save_image_path, "wb") as f:
            f.write(pdf_file.getbuffer())
        show_pdf(save_image_path)
        # Specify the path to your config file
        #config_path = "C:/LANGUAGE_SUPPORTING_TOOLS/python/Lib/site-packages/en_core_web_sm/en_core_web_sm-3.7.1/config.cfg"

        # Initialize ResumeParser with the resume file and config file
        resume_parser = ResumeParser(save_image_path)
        ...
        if resume_data:
            ...
            if resume_data['no_of_pages'] == 1:
                ...
                st.markdown('''<h4 style='text-align: left; color: #1ed760;'> You are at Itermediate level.</h4>''',unsafe_allow_html=True)
            elif resume_data['no_of_pages'] >=3:
                cand_level = "Experienced"
                st.markdown('''<h4 style='text-align: left; color: #fba171;'> You are Experienced.</h4>''',unsafe_allow_html=True)
            ...
            #recommendation
            ds_keyword = ['tensorflow', 'keras', 'pytorch', 'machine learning', 'deep learning', 'flask',' Streamlit','Data Visualization','Supervised Learning','Unsupervised Learning']
            ...
            for i in resume_data['skills']:
                ...
                if i.lower() in ds_keyword:
                    ...
                    recommended_skills = ['Data Visualization', 'Predictive Analysis', 'Statistical Modeling',
                                              ...
                                              'Streamlit','Supervised Learning','Unsupervised Learning']
                    recommended_keywords = st_tags(label='### Recommended skills for you.',
                                                   text='Recommended skills generated from System',
                                                   value=recommended_skills, key='2')
                    st.markdown(
                        '''<h4 styles='text-align: left; color: #1ed760;'>Adding this skills to resume will boost🚀 the chance of getting a Job💼</h4>''',unsafe_allow_html=True)
                ...
                elif i.lower() in 'Android Development':
                    print(i.lower())
                    reco_field = 'Android Development'
                    st.success("**Our analysis says you are looking for Android App Development Jobs **")
                    recommended_skills = ['Android', 'Android development', 'Flutter', 'Kotlin', 'XML', 'Java','Kivy', 'GIT', 'SDK', 'SQLite']
                    recommended_keywords = st_tags(label ='### Recommended skills for you',
                                                   text= 'Recommended skills genrated from System',
            ...
            if 'Objective' in resume_text:
                ...
                if st.button("Analyze Video"):
                    video_title = fetch_yt_video(resume_vid)
                    if video_title:
                        st.success(f"Video title: {video_title}")
                        sub_header = st.subheader("✅ **" + video_title + "**")
                    
                    else:
                        st.error("Error fetching video information. Please make sure you've entered a valid YouTube link.")
                    
                inter_vid_title = fetch_yt_video(interview_videos)
                ...
                #res_sub_header = st.subheader("✅ **" + res_vid_title + "**")
                #res_sub_header
                st.video(resume_vid)
    ...
    if st.button('Login'):
        if ad_user == 'KESHAV' and ad_password == 'KES90@pk':
            st.success("Welcome keshav")
            # Display Data
